...
describe("Ragequit E2E Tests", function () {
  ...
  describe("Basic Ragequit Scenarios", function () {
    it("Should allow member to ragequit with shares only", async function () {
      const { baal, shares, loot, avatar, alice } = await loadFixture(deployDAOWithTreasury);
      ...
      const tx = await baal.connect(alice).ragequit(
        alice.address,
        sharesToBurn,
        lootToBurn,
      ...
      // Verify Ragequit event (member, to, lootToBurn, sharesToBurn, tokens)
      await expect(tx)
        .to.emit(baal, "Ragequit")
        .withArgs(
    ...
    it("Should allow member to ragequit with loot only", async function () {
      const { baal, loot, avatar, alice } = await loadFixture(deployDAOWithTreasury);
      ...
      // Verify event (member, to, lootToBurn, sharesToBurn, tokens)
      await expect(tx)
        .to.emit(baal, "Ragequit")
        .withArgs(alice.address, alice.address, lootToBurn, 0, [ethers.ZeroAddress]);
    ...
    it("Should allow member to ragequit with both shares and loot", async function () {
      ...
      await expect(tx)
        .to.emit(baal, "Ragequit")
        .withArgs(alice.address, alice.address, lootToBurn, sharesToBurn, [ethers.ZeroAddress]);
  ...
  describe("Fair Share Calculations", function () {
    it("Should calculate fair share correctly", async function () {
      const { baal, shares, loot, avatar, alice, bob } = await loadFixture(deployDAOWithTreasury);
      ...
      await baal.connect(alice).ragequit(alice.address, sharesToBurn, 0, [ethers.ZeroAddress]);
    ...
    it("Should handle multiple members ragequitting in sequence", async function () {
      const { baal, shares, avatar, alice, bob } = await loadFixture(deployDAOWithTreasury);
      ...
      await baal.connect(alice).ragequit(alice.address, ethers.parseEther("50"), 0, [ethers.ZeroAddress]);
      ...
      await baal.connect(bob).ragequit(bob.address, ethers.parseEther("30"), 0, [ethers.ZeroAddress]);
  ...
  describe("Multiple Guild Tokens", function () {
    it("Should ragequit with ETH (guild token functionality)", async function () {
      const { baal, shares, avatar, alice } = await loadFixture(deployDAOWithTreasury);
      ...
      await baal.connect(alice).ragequit(
        alice.address,
  ...
  describe("Retention Percentage Enforcement", function () {
    it("Should enforce minimum retention percentage", async function () {
      const { baal, shares, loot, alice } = await loadFixture(deployDAOWithTreasury);
      ...
      if (aliceTotal > maxBurn) {
        ...
        await expect(
          baal.connect(alice).ragequit(alice.address, aliceShares, aliceLoot, [ethers.ZeroAddress])
      ...
      await baal.connect(alice).ragequit(alice.address, safeBurnAmount, 0, [ethers.ZeroAddress]);
    ...
    it("Should allow ragequit up to but not exceeding retention limit", async function () {
      ...
      if (safeBurn > 0n && safeBurn <= await shares.balanceOf(alice.address)) {
        await expect(
          baal.connect(alice).ragequit(alice.address, safeBurn, 0, [ethers.ZeroAddress])
        ).to.not.be.reverted;
  ...
  describe("Edge Cases and Validation", function () {
    it("Should reject ragequit with zero amount", async function () {
      const { baal, alice } = await loadFixture(deployDAOWithTreasury);

      await expect(
        baal.connect(alice).ragequit(alice.address, 0, 0, [ethers.ZeroAddress])
      ).to.be.reverted;
    ...
    it("Should reject ragequit with insufficient balance", async function () {
      ...
      await expect(
        baal.connect(alice).ragequit(alice.address, excessiveAmount, 0, [ethers.ZeroAddress])
      ).to.be.reverted;
    ...
    it("Should reject ragequit with duplicate tokens", async function () {
      ...
      await expect(
        baal.connect(alice).ragequit(
          alice.address,
          ethers.parseEther("10"),
          0,
          [ethers.ZeroAddress, ethers.ZeroAddress] // Duplicate!
        )
      ).to.be.revertedWith("Baal: duplicate token");
    ...
    it("Should send funds to specified recipient (not just msg.sender)", async function () {
      const { baal, shares, avatar, alice, carol } = await loadFixture(deployDAOWithTreasury);
  ...
  describe("Event Validation", function () {
    it("Should emit correct Ragequit event parameters", async function () {
      ...
      const parsed = baal.interface.parseLog(event!);
