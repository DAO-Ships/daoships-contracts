...
describe("E2E: Deployed DAO", function () {
  ...
  before(async function () {
    ...
    BaalSummonerABI = JSON.parse(
      fs.readFileSync(path.join(artifactsDir, "core/BaalSummoner.sol/BaalSummoner.json"), "utf-8")
    ).abi;
  ...
  describe("Deployment Verification", function () {
    ...
    it("Should verify BaalSummoner singleton references", async function () {
      const address = deploymentAddresses.contracts.BaalSummoner;
      const baalSummoner = new quais.Contract(address, BaalSummonerABI, deployer);

      // Warm up provider
      await provider.getBlockNumber(Shard.Cyprus1);

      const baalTemplate = await baalSummoner.baalSingleton();
      const sharesTemplate = await baalSummoner.sharesSingleton();
      const lootTemplate = await baalSummoner.lootSingleton();

      expect(baalTemplate.toLowerCase()).to.equal(
        deploymentAddresses.contracts.BaalSingleton.toLowerCase()
      );
      ...
      console.log(`    ✓ BaalSummoner templates correctly configured`);
    });

    it("Should verify BaalAndVaultSummoner references", async function () {
      const address = deploymentAddresses.contracts.BaalAndVaultSummoner;
      const summoner = new quais.Contract(address, BaalAndVaultSummonerABI, deployer);

      // Warm up provider
      await provider.getBlockNumber(Shard.Cyprus1);

      const baalSummonerRef = await summoner.baalSummoner();
      const quaiVaultFactory = await summoner.quaiVaultFactory();

      expect(baalSummonerRef.toLowerCase()).to.equal(
        deploymentAddresses.contracts.BaalSummoner.toLowerCase()
      );
      expect(quaiVaultFactory).to.match(/^0x[0-9a-fA-F]{40}$/);

      console.log(`    ✓ BaalAndVaultSummoner:`);
      console.log(`      - BaalSummoner: ${baalSummonerRef}`);
  ...
  describe("Summoned DAO Verification", function () {
    ...
    it("Should verify summoned DAO contracts exist", async function () {
      for (const dao of summonedDAOs) {
        ...
        const contracts = [
          { name: "Baal", address: dao.contracts.baal },
  ...
  describe("BaalSummoner Calculate Addresses", function () {
    it("Should calculate deterministic addresses", async function () {
      const summonerAddress = deploymentAddresses.contracts.BaalSummoner;
      const summoner = new quais.Contract(summonerAddress, BaalSummonerABI, deployer);
      ...
      const [baal, shares, loot] = await summoner.calculateAddresses(
        sender,
        sharesSalt,
        lootSalt,
        baalSalt
    ...
    it("Should calculate same addresses for same inputs", async function () {
      ...
      const [baal1, shares1, loot1] = await summoner.calculateAddresses(
        sender,
        sharesSalt,
        lootSalt,
        baalSalt
      ...
      const [baal2, shares2, loot2] = await summoner.calculateAddresses(
        sender,
        sharesSalt,
        lootSalt,
        baalSalt
