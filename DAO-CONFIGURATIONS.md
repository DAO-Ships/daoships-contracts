...
- **Guild token limits:** Keep the guild token list under **50 tokens**. Removing guild tokens via governance proposal scans the list linearly (~2,100 gas per token). With 50 tokens this costs ~105K gas per removal — well within limits. Duplicate addresses passed during DAO creation are automatically deduplicated.
...
- Guild tokens must include every asset the fund holds — members ragequit into proportional slices of the entire portfolio
